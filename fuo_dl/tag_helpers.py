...
def cook_tagobj(song, album, artists, refine_tagobj_func=None):
    tag_obj = {"title": song.title, "artist": song.artists_name}
...
def cook_filepath_old(tag_obj, ext):
    ...
    if tag_obj.get("album"):
        storage_path = os.path.join(
            correct_str(tag_obj["albumartist"]), correct_str(tag_obj["album"])
        )
        extra_name = ""
        if tag_obj.get("discnumber") and tag_obj.get("tracknumber"):
            extra_name = "{:0>2d} ".format(int(tag_obj["tracknumber"].split("/")[0]))
            if (
                len(tag_obj["discnumber"].split("/")) > 1
                and int(tag_obj["discnumber"].split("/")[1]) > 1
            ):
                extra_name = "{}-{}".format(
                    tag_obj["discnumber"].split("/")[0], extra_name
                )
...
def cook_filepath(tag_obj, ext, fmts):
    ...
    # 目前支持获取的tag字段和计算方法
    valid_tag_map = dict()
    if tag_obj.get("albumartist"):
        valid_tag_map["albumartist"] = correct_str(tag_obj["albumartist"])
    if tag_obj.get("album"):
        valid_tag_map["album"] = correct_str(tag_obj["album"])
    if tag_obj.get("artist"):
        valid_tag_map["artist"] = correct_str(tag_obj["artist"])
    if tag_obj.get("title"):
        valid_tag_map["title"] = correct_str(tag_obj["title"])
    if tag_obj.get("discnumber") and tag_obj.get("tracknumber"):
        valid_tag_map["track"] = "{:0>2d}".format(
            int(tag_obj["tracknumber"].split("/")[0])
        )
        if (
            len(tag_obj["discnumber"].split("/")) > 1
            and int(tag_obj["discnumber"].split("/")[1]) > 1
            ...
            valid_tag_map["track"] = "{}-{}".format(
                tag_obj["discnumber"].split("/")[0], valid_tag_map["track"]
            )
